...
def main(raw_args=None):
	parser = argparse.ArgumentParser(description='SV-Annotate v0.1', formatter_class=argparse.ArgumentDefaultsHelpFormatter,)
	...
	#
	parser.add_argument('--readcount',     type=int,   required=False, metavar='[2]',   help="Initial readcount filter",          default=2)
	parser.add_argument('--readcount-bnd', type=int,   required=False, metavar='[5]',   help="BND readcount filter",              default=5)
	parser.add_argument('--vaf',           type=float, required=False, metavar='[0.2]', help="Initial VAF filter",                default=0.2)
	parser.add_argument('--vaf-bnd',       type=float, required=False, metavar='[0.3]', help="BND VAF filter",                    default=0.3)
	parser.add_argument('--control-freq',  type=float, required=False, metavar='[0.3]', help="Minimum SV frequency in controls",  default=0.5)
	parser.add_argument('--min-size',      type=int,   required=False, metavar='[50]',  help="Minimum size (non-repetitive SVs)", default=50)
	parser.add_argument('--min-size-rep',  type=int,   required=False, metavar='[300]', help="Minimum size (repetitive SVs)",     default=300)
	...
	#
	BED_DICT_GENE = parse_bed_regions(res_path + 'gencodeV26_gene.bed.gz', 4)
	BED_DICT_EXON = parse_bed_regions(res_path + 'gencodeV26_exon.bed.gz', 4)
	COMMON_SVS    = get_common_svs(res_path + 'common_svs_hg38.bed.gz')
	CONTROL_SVS   = get_common_svs(res_path + 'control_svs_hg38.bed.gz', min_control_freq=CONTROL_FREQ)
	REPEAT_BED    = get_repetitive_regions(res_path + 'hg38_simpleRepeats.bed.gz')
	REPEAT_TRACK  = MappabilityTrack(res_path + 'hg38_simpleRepeats.bed.gz', bed_buffer=REP_BUFF)
